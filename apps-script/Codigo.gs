...
function gravarCusto(p) {
  ...
  // 1) Resumo do projeto
  ensureSheet(ABA_PROJETOS, [
    "Data", "ID", "Filamento", "Custo Filamento", "Custo Energia",
    "Mão de Obra", "Custos Fixos", "Insumos", "Custo Total",
    "Margem %", "Preço Sugerido", "Lucro Estimado",
  ]).appendRow([
    agora, p.projetoId, p.filamento,
    num(c.filamento), num(c.energia), num(c.maoDeObra),
    num(c.custosFixos), num(c.insumos), num(p.custoTotal),
    num(p.margem), num(p.precoSugerido), num(p.lucroEstimado),
  ]);

  // 2) Custo de Filamento
  ensureSheet(ABA_FILAMENTO_CUSTO, [
    "Data", "ID", "Filamento", "Preço/Kg", "Qtd (g)", "Custo",
  ]).appendRow([
    agora, p.projetoId, p.filamento,
  ...
  ensureSheet(ABA_ENERGIA, [
    "Data", "ID", "Consumo (W)", "Tempo (h)", "kWh", "Custo",
  ]).appendRow([
    agora, p.projetoId, num(p.consumoW), num(p.horas), num(p.valorKwh), num(c.energia),
  ]);
